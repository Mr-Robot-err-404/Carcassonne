...
export default function Node({ isNodeCenter, row, col, tile, isRecentTile }: NodeProps) {
    ...
    return (
        <div className="relative">
            ...
            className={`h-full bg-transparent w-20 hover:border hover:border-transparent ${isRecentTile && isClaimPossible(claims) && !isNodeClaimed && isClaimReady && 'border-4 border-yellow-500'} ${playerTerritory[row][col].claims.length && "border-2 border-green-400"} ${opponentTerritory[row][col].claims.length && "border-2 border-orange-500"} ${playerTerritory[row][col].claims.length && opponentTerritory[row][col].claims.length && "border-2 border-purple-500"}  ${isOver ? "border-green-500 border-2" : ""} flex-shrink-0`}>
            {tile.img && 
                <Image
